 ...
 * June 9, 2024
 * Derived from
i2c_controller_rw_multibyte_fifo_poll_LP_MSPM0G3507_nortos_ticlang Educational
BoosterPack MKII (BOOSTXL-EDUMKII) J1.8 ambient light (OPT3001) interrupt
(digital)  PA27 J1.9 ambient light (OPT3001) and temperature sensor (TMP006) I2C
SCL (I2C)  PB2_I2C1-SCL J1.10 ambient light (OPT3001) and temperature sensor
(TMP006) I2C SDA (I2C) PB3_I2C1-SDA J2.11 temperature sensor (TMP006) interrupt
(digital) PB16 The TMP006 was discontinued in 2017. Therefore, it is no longer
populated on the board. Documentation of the TMP006 is unchanged in this user's
guide to accommodate users who still have the device populated on the board
SSD1306 oLED
  VCC to +3.3V
...
int static IC2FillTxFifo(uint8_t *buffer, uint16_t count) {
  for (int i = 0; i < count; i++) {
    if ((I2C1->MASTER.MFIFOSR & 0x0F00) == 0)
      return 0; // fail TxFifo can't take data
...
int I2C_Send1(int8_t slave, uint8_t data1) {
  ...
  // bit 1 START=1
  // bit 0 BURSTRUN=1
  while ((I2C1->MASTER.MSR & 0x01) == 0x01) {
  }; // wait until not busy
...
int I2C_Send(uint8_t slave, uint8_t *pData, uint32_t count) {
  ...
  if (IC2FillTxFifo(pData, count) == 0)
    return 0;
  while ((I2C1->MASTER.MSR & 0x20) == 0) {
  }; // wait until idle
  I2C1->MASTER.MSA = (slave << 1);
...
int I2C_SendData(uint8_t slave, uint8_t *pData, uint32_t count) {
  int i;
  if ((I2C1->MASTER.MFIFOSR & 0x0F00) == 0)
    return 0;                  // fail TxFifo full
  I2C1->MASTER.MTXDATA = 0x40; // 0x40 means data
  if (count > 7) {
    if (IC2FillTxFifo(pData, 7) == 0)
      return 0; // first 8
    i = 7;
  } else {
    if (IC2FillTxFifo(pData, count) == 0)
      return 0; // all data should fit
    i = count;
  }
  ...
  // bit 0 BURSTRUN=1
  while (i < count) {
    while ((I2C1->MASTER.MFIFOSR & 0x0F00) == 0) {
    };                               // wait for room
...
void I2C_LightSensor_Init(void) {
  I2C_Init();
  IOMUX->SECCFG.PINCM[PA27INDEX] =
      0x00040081; // GPIO input (OPT3001) interrupt (digital)
}
...
uint32_t I2C_LightSensor_Input(void) {
  ...
  while ((GPIOA->DIN31_0 & LIGHTINT) == LIGHTINT) {
...
void I2C_TempSensor_Init(void) {
  ...
  IOMUX->SECCFG.PINCM[PB16INDEX] =
      0x00040081; // GPIO input, (TMP006) interrupt (digital)
}
...
void I2C_TempSensor_Input(int32_t *sensorV, int32_t *localT) {
  ...
  while ((GPIOB->DIN31_0 & TEMPINT) == TEMPINT) {
  }; // wait for conversion to complete
...
// test WhoAmI register - should output 0x67
uint8_t IMU_WhoAmI(int8_t slave) {
  I2C_Send1(slave, 0x75); // whoami register
  uint8_t recv_data = I2C_Recv1(slave);
  return recv_data;
}

// initialize IMU
void IMU_Init(int8_t slave) {
  I2C_Send2(slave, 0x02,
            0xFF);     // register 0x02 (SIGNAL_PATH_RESET) - soft reset
  Clock_Delay1ms(100); // 100ms delay for reset
  I2C_Send2(slave, 0x1F, 0x0F); // register 0x1F (PWR_MGMT0), data 0x0F - set
                                // gyro/accel to low noise mode
  Clock_Delay1ms(100);          // 100ms delay for power management
  I2C_Send2(slave, 0x20, 0x65); // register 0x20 (GYRO_CONFIG0), data 0x68 - set
                                // gyro range to +-250 dps, ODR to 200 Hz
  I2C_Send2(slave, 0x23, 0x07); // register 0x23 (GPIO_CONFIG1), data 0x07 - low
                                // pass filter type shi
  // I2C_Send2(slave, 0x21, 0x06); // register 0x21 (ACCEL_CONFIG0), data 0x08 -
  //                               // set accel range to +-16g, ODR to 200 Hz
  Clock_Delay1ms(100); // wait 100ms
}

// query z-axis data (3 samples averaging)
int16_t IMU_Z_Axis(int8_t slave) {
  int16_t angular_velocity = 0; // reset gyro data
  int8_t recv_data1 = 0;
  int8_t recv_data0 = 0;
  ...
  for (uint8_t i = 0; i < 3; ++i) {
    ...
    I2C_Send1(
        slave,
        0x15); // register 0x15 (GYRO_DATA_Z1) - upper byte of Gyro Z-axis data
    recv_data1 = I2C_Recv1(slave);
    recv_data0 = I2C_Recv1(slave);

    angular_velocity = (((recv_data1 << 8) + recv_data0) - GYRO_OFFSET) /
                       131; // query IMU for z-axis data
  }
